    ...
    <BaseDrawer.Portal>
      ...
      <BaseDrawer.Viewport className="pointer-events-none fixed inset-0 overflow-hidden">
        <BaseDrawer.Popup
          {...props}
          className={cn(
            "absolute w-[calc(100%-2*var(--drawer-gap))] rounded-sm [--drawer-gap:--spacing(2)] sm:[--drawer-gap:--spacing(4)] transition-[transform,height] duration-300 ease-[ease] [--stack-depth:var(--nested-drawers,0)] [--stack-scale:max(0.8,calc(1-var(--stack-depth)*0.04))] [--stack-offset:calc(min(var(--stack-depth),5)*1rem)]",
            "data-[swipe-direction=right]:inset-y-(--drawer-gap) data-[swipe-direction=right]:right-(--drawer-gap) data-[swipe-direction=right]:origin-right data-[swipe-direction=right]:[transform:translateX(calc(var(--drawer-swipe-movement-x,0px)-var(--stack-offset)-(1-var(--stack-scale))*100%))_scale(var(--stack-scale))]",
            "data-[swipe-direction=left]:inset-y-(--drawer-gap) data-[swipe-direction=left]:left-(--drawer-gap) data-[swipe-direction=left]:origin-left data-[swipe-direction=left]:[transform:translateX(calc(var(--drawer-swipe-movement-x,0px)+var(--stack-offset)+(1-var(--stack-scale))*100%))_scale(var(--stack-scale))]",
            "data-[swipe-direction=down]:inset-x-0 data-[swipe-direction=down]:bottom-(--drawer-gap) data-[swipe-direction=down]:mx-auto data-[swipe-direction=down]:h-[var(--drawer-height,auto)] data-[swipe-direction=down]:max-h-[calc(100%-2*var(--drawer-gap))] data-[swipe-direction=down]:origin-bottom data-[swipe-direction=down]:[transform:translateY(calc(var(--drawer-snap-point-offset,0px)+var(--drawer-swipe-movement-y,0px)-var(--stack-offset)-(1-var(--stack-scale))*100%))_scale(var(--stack-scale))]",
            "data-[swipe-direction=down]:data-nested-drawer-open:h-[var(--drawer-frontmost-height,var(--drawer-height,auto))]",
            "data-[swipe-direction=right]:data-starting-style:[transform:translateX(calc(100%+var(--drawer-gap)))] data-[swipe-direction=right]:data-ending-style:[transform:translateX(calc(100%+var(--drawer-gap)))] data-[swipe-direction=left]:data-starting-style:[transform:translateX(calc(-100%-var(--drawer-gap)))] data-[swipe-direction=left]:data-ending-style:[transform:translateX(calc(-100%-var(--drawer-gap)))] data-[swipe-direction=down]:data-starting-style:[transform:translateY(calc(100%+var(--drawer-gap)))] data-[swipe-direction=down]:data-ending-style:[transform:translateY(calc(100%+var(--drawer-gap)))]",
