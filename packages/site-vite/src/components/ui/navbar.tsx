...
export function NavbarDivider({ className, ...props }: React.ComponentPropsWithoutRef<"div">) {
  return <div aria-hidden="true" {...props} className={clsx(className, "h-6 w-px bg-white/10")} />;
}

export function NavbarSection({ className, ...props }: React.ComponentProps<"div">) {
  return <NavigationSection {...props} className={clsx(className, "flex items-center gap-3")} />;
}
