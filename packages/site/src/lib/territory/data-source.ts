...
export class TerritoryDataSource {
  ...
  readonly productionRates: Record<TerritoryResourcePoint["kind"], number>;
  readonly definitions: TerritoryMeshDefinition[];
  readonly provinceGrid: ProvinceRestrictionGrid | null;
  readonly baseUrl: string;
  readonly image: HTMLImageElement;
  readonly chunks = new Map<string, TerritorySpatialChunk>();
  readonly chunkPromises = new Map<string, Promise<TerritorySpatialChunk | null>>();
  ...
  static async load(mapSlug: string, signal?: AbortSignal): Promise<TerritoryDataSource> {
    ...
    const province = provinceBuffer ? await decodeContainer(provinceBuffer, 403) : null;
    return new TerritoryDataSource(
      config,
      await decodeContainer(definitionsBuffer, 401),
      province
        ? {
            ...province,
            flagBlocked: new Set(province.flagBlocked),
            fortressBlocked: new Set(province.fortressBlocked),
          }
        : null,
      baseUrl,
      image
  ...
  async loadChunk(
    ...
    y: number,
    signal?: AbortSignal
  ): Promise<TerritorySpatialChunk | null> {
    ...
    const promise = fetchBuffer(`${this.baseUrl}${path}`, signal)
      .then(async (buffer) => {
        const chunk = await decodeContainer(buffer, 402);
  ...
  chunksWithin(
    ...
    return this.chunkCoordinates(bounds, buffer)
      ...
      .filter((chunk): chunk is TerritorySpatialChunk => Boolean(chunk));
  }

  allLoadedResources(): TerritoryResourcePoint[] {
    const resources: TerritoryResourcePoint[] = [];
    for (const chunk of this.chunks.values()) resources.push(...chunk.resources);
    return resources;
  }

  allLoadedStructures(): TerritoryStructure[] {
    const structures: TerritoryStructure[] = [];
