...
import type { BuildingKind, PlannedBuilding, ProvinceRestrictionGrid } from "./types";
...
export function structureTerritoryCells(
  structure: Pick<TerritoryStructure, "x" | "y" | "territoryRadiusInCells">
): TerritoryCell[] {
...
export function transformedTriangle(
  definition: TerritoryMeshDefinition,
  instance: TerritoryMeshInstance,
  index: number
...
export function countCoveredResources(
  ...
  allianceId: string
): Record<TerritoryResourceKind, number> {
  const counts: Record<TerritoryResourceKind, number> = {
    food: 0,
    wood: 0,
    stone: 0,
    coin: 0,
...
export function calculateResourceProduction(
  counts: Record<TerritoryResourceKind, number>,
  rates: Record<TerritoryResourceKind, number>
): Record<TerritoryResourceKind, number> {
  return Object.fromEntries(
    (Object.entries(counts) as Array<[TerritoryResourceKind, number]>).map(([kind, count]) => [
      kind,
      count * rates[kind],
    ])
  ) as Record<TerritoryResourceKind, number>;
}
