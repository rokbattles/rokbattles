...
fn print_success(raw_only: bool, output_paths: &io::OutputPaths) {
    ...
    if let Some(processed_v2) = &output_paths.processed_v2 {
        println!(
            "Successfully wrote files to: '{}', '{}', and '{}'",
            output_paths.raw.display(),
            output_paths.processed.display(),
            processed_v2.display()
        );
    } else {
        println!(
            "Successfully wrote files to: '{}' and '{}'",
...
mod tests {
    ...
    fn determine_output_paths_dir_without_v2() {
        ...
        let paths = io::determine_output_paths(&output, "mail-1", false).expect("paths");

        assert_eq!(paths.raw, PathBuf::from("out/mail-1.json"));
        assert_eq!(paths.processed, PathBuf::from("out/mail-1-processed.json"));
        assert!(paths.processed_v2.is_none());
    }

    #[test]
    fn determine_output_paths_dir_with_v2() {
        let output = PathBuf::from("out");
        let paths = io::determine_output_paths(&output, "mail-2", true).expect("paths");

        assert_eq!(paths.raw, PathBuf::from("out/mail-2.json"));
        assert_eq!(paths.processed, PathBuf::from("out/mail-2-processed.json"));
        assert_eq!(
            paths.processed_v2,
            Some(PathBuf::from("out/mail-2-processed-v2.json"))
        );
    }
    ...
    fn determine_output_paths_file_with_v2() {
        ...
        let paths = io::determine_output_paths(&output, "mail-3", true).expect("paths");

        assert_eq!(paths.raw, PathBuf::from("out/mail.json"));
        assert_eq!(paths.processed, PathBuf::from("out/mail-processed.json"));
        assert_eq!(
            paths.processed_v2,
            Some(PathBuf::from("out/mail-processed-v2.json"))
        );
    }
    ...
    fn processed_filename_detection() {
        ...
        assert!(io::is_processed_filename("foo-processed-v2.json"));
    ...
    fn classify_path_skips_processed_outputs() {
        ...
        assert!(dir::classify_path(PathBuf::from("foo-processed-v2.json").as_path()).is_none());
    }
