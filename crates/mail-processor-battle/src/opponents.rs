    ...
    fn extract(&self, input: &Value) -> Result<Section, ExtractError> {
        ...
        std::thread::scope(|scope| {
            ...
            for (attack_id, attack) in attacks {
                let attack_id = attack_id.to_string();
                let handle = scope.spawn(move || extract_attack_entry(&attack_id, attack));
                handles.push(handle);
            }
        ...
        results.sort_by_key(|(attack_id, _)| *attack_id);
        let entries = results.into_iter().map(|(_, value)| value).collect();
...
fn extract_attack_entry(attack_id: &str, attack: &Value) -> Result<(u64, Value), ExtractError> {
    ...
    let attack_id = parse_attack_id(attack_id)?;
    ...
    fields.insert(
        "attack".to_string(),
        json!({ "id": attack_id, "x": attack_x, "y": attack_y }),
    );
    ...
    Ok((attack_id, Value::Object(fields)))
}
