...
impl RuntimeArtifact {
    ...
    pub fn load_default() -> Result<Self, ArtifactError> {
        ...
        Self::load(
            &Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("../../apps/rokbattles-nat-gateway")
                .join(runtime_path),
        )
    }

    /// Read and validate the protocol artifact at an explicit path.
    ///
    /// # Errors
    /// Returns [`ArtifactError`] for I/O, size, JSON or protocol-schema failures.
    ...
    fn from_file(file: RuntimeArtifactFile) -> Result<Self, ArtifactError> {
        ...
        let protocol = ProtocolSchema {
            ...
            mail_server_id_field: mail_entity.required_field("ServerId", FieldType::Int32)?.number,
    ...
    /// Minimal non-production schema for synthetic transport tests.
    ///
            ...
            protocol: ProtocolSchema {
                ...
                mail_entity: MessageShape {
                    fields: HashMap::from([
                        ...
                        (13, WireRule { primary: 0, packed: false }),
                    ]),
                },
            },
            carriers,
...
mod tests {
    ...
    fn fixture_path(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir()
            .join(format!("rokbattles-gateway-artifact-{name}-{}-{nonce}.json", std::process::id()))
    }
}
