...
/// Extracts reward entries out of AllianceAOOBattleInfo attachments.
...
mod tests {
    ...
    fn rewards_extractor_reads_all_attachments() {
        let input = json!({
            "attachments": [
                { "loot": [{ "Type": 2, "SubType": 30, "Value": 1 }] },
                { "loot": [{ "Type": 2, "SubType": 44, "Value": 2 }] }
            ]
        });
        ...
        assert_eq!(rewards.len(), 2);
        assert_eq!(rewards[0]["sub_type"], json!(30));
        assert_eq!(rewards[1]["sub_type"], json!(44));
    }
    ...
    fn roundtrip_rewards_extracts_sample() {
        ...
        assert_eq!(rewards[0], json!({"type": 2, "sub_type": 30, "value": 1}));
        assert_eq!(rewards[5], json!({"type": 2, "sub_type": 405, "value": 20}));
    }
}
