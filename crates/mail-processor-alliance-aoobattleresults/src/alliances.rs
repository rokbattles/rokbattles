...
use crate::content::{
    require_body_kvs, require_bool_field, require_string_field, require_u64_field,
};
...
impl Extractor for AlliancesExtractor {
    ...
    fn extract(&self, input: &Value) -> Result<Section, ExtractError> {
        ...
        for entry in as_infos {
            ...
            let abbreviation = require_string_field(entry, "Abbr")?;
            let id = require_u64_field(entry, "AllianceId")?;
            let name = require_string_field(entry, "Name")?;
            let logo = require_string_field(entry, "Logo")?;
            let members = require_u64_field(entry, "Members")?;
            let members_max = require_u64_field(entry, "MemberMax")?;
            let power = require_u64_field(entry, "Power")?;
            let score = require_u64_field(entry, "Score")?;
            let server_id = require_u64_field(entry, "ServerId")?;
            let is_blue = require_bool_field(entry, "IsBlue")?;
            let team = require_u64_field(entry, "Idx")?;
