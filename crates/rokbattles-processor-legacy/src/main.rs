...
async fn process_mail(
    ...
    let mail_type =
        mail_helper_legacy::detect_mail_type(&decoded_mail).context("missing mail type")?;
    ...
    let docs = match mail_type {
        EmailType::Battle => {
            let mail_obj = mail_processor_battle_legacy::process_sections(&decoded_mail.sections)?;
        ...
        EmailType::DuelBattle2 => {
            let processed_mail =
                mail_processor_duelbattle2_legacy::process_sections(&decoded_mail.sections)?;
