...
pub async fn ingress(State(st): State<AppState>, req: Request<Body>) -> impl IntoResponse {
    ...
    let mail_type_raw = mail_helper_legacy::detect_mail_type_str(&decoded_mail);
    ...
    let mail_time = match mail_helper_legacy::detect_mail_time(&decoded_mail) {
        Some(t) => t,
        None => {
    ...
    let mail_id = match mail_helper_legacy::detect_email_id(&decoded_mail) {
        Some(id) => id,
        None => {
