//! Metadata parser for BarCanyonKillBoss mail.

use mail_sdk::{ExtractError, Extractor, Section, extract_base_metadata};
use serde_json::Value;

/// Pulls top-level metadata out of a BarCanyonKillBoss mail.
...
mod tests {
    ...
    fn roundtrip_metadata_extracts_sample() {
        let sample_path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(
            "../../../../samples/BarCanyonKillBoss/Persistent.Mail.21162669176948646831.json",
        );
        ...
        let fields = section.fields();
        assert_eq!(fields["mail_id"], json!("21162669176948646831"));
        assert_eq!(fields["mail_receiver"], json!("player_71738515"));
        assert_eq!(fields["server_id"], json!(1804));
        assert_eq!(fields["mail_time"], json!(1769486468622942u64));
    }
}
