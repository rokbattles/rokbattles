//! Metadata extractor for SystemBarbarianFort mail.

use mail_processor_sdk::{ExtractError, Extractor, Section, require_string, require_u64};
use serde_json::Value;

/// Extracts top-level metadata fields from a SystemBarbarianFort mail.
...
mod tests {
    ...
    fn roundtrip_metadata_extracts_sample() {
        ...
        let fields = section.fields();
        assert_eq!(fields["mail_id"], json!("87938122177133895831"));
        assert_eq!(fields["mail_receiver"], json!("player_71738515"));
        assert_eq!(fields["server_id"], json!(1804));
        assert_eq!(fields["mail_time"], json!(1771338958619245u64));
    }
}
