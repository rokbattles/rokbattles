...
use rokbattles_api::{
    build_router,
    config::Config,
    db::{DnsCheckStore, GameLocalizationStore, MongoAuthStore, ReportsStore},
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dns_check_store = DnsCheckStore::new(database.clone());
    dns_check_store.ensure_indexes().await?;
