...
use mail_helper_legacy::EmailType;
...
pub(crate) fn process_decoded_mail(
    ...
    let mail_type = mail_helper_legacy::detect_mail_type(&decoded.decoded_mail);
    let output_paths = io::determine_output_paths(output, &decoded.id)?;
    ...
    let processed_mail_json = match mail_type {
        Some(EmailType::DuelBattle2) => {
            let processed_mail = mail_processor_duelbattle2_legacy::process_sections(
                &decoded.decoded_mail.sections,
            )?;
            serde_json::to_value(processed_mail)?
        }
        _ => {
            let processed_mail =
                mail_processor_battle_legacy::process_sections(&decoded.decoded_mail.sections)?;
            serde_json::to_value(processed_mail)?
        }
    };
