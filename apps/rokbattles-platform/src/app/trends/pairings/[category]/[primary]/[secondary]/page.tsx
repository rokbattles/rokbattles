...
export default async function Page({
  ...
  const [t, tNav] = await Promise.all([getTranslations("trends"), getTranslations("navigation")]);
  ...
  if (!Number.isFinite(primaryId) || !Number.isFinite(secondaryId)) {
    return <Text className="mt-6 text-sm text-zinc-500">{t("states.invalidPairing")}</Text>;
  }
