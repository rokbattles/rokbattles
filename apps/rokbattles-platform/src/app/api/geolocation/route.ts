...
const isGDPR = (countryCode: string | null = null): boolean => {
  return countryCode ? gdprList.indexOf(countryCode) > -1 : true;
};

export function GET(req: NextRequest) {
  const country = req.headers.get("CF-IPCountry");
  const consent = isGDPR(country);

  return NextResponse.json({ country, isGDPR: consent });
}
