...
function extractSummary(payload: RawReportPayload) {
  ...
  const battleResults = payload.battle_results as
    | RawBattleResults
    | null
    | undefined;
  return {
...
export default async function Image({
  ...
  try {
    ...
    const chosen =
      reports.find((entry) => hasOverviewData(entry.payload.overview)) ??
      reports[0] ??
      null;
    ...
    const selfPrimaryCommander =
      getCommanderName(payload.self?.primary_commander?.id) ??
      tCommon("labels.unknown");
    const selfSecondaryCommander = getCommanderName(
      payload.self?.secondary_commander?.id
    );

    const enemyPrimaryCommander =
      getCommanderName(payload.enemy?.primary_commander?.id) ??
      tCommon("labels.unknown");
    const enemySecondaryCommander = getCommanderName(
      payload.enemy?.secondary_commander?.id
    );
    ...
    const image = new ImageResponse(
      <div tw="w-full h-full flex flex-col bg-zinc-900 text-white">
        <div tw="w-full pt-14 px-16 flex items-center justify-start">
          <div tw="text-5xl font-semibold tracking-tight">
            {t("battleReportTitle")}
          </div>
        </div>
        <div tw="flex-1 w-full flex items-start justify-between px-16 pt-14">
          <div tw="flex flex-col max-w-[520px]">
            <div tw="text-6xl font-semibold leading-none">
              {selfPrimaryCommander}
            </div>
            ...
            <div tw="mt-10 flex flex-col text-3xl text-zinc-200">
              ...
              <div tw="mt-3">
                {formatInteger(
                  summary.selfKillPoints,
                  t("og.killPointsSuffix"),
                  fallbackLabel
                )}
              </div>
              <div tw="mt-3">
