...
export function ReportParticipantCard({
  ...
  const artifactTokens = showArtifacts
    ? parseEquipment(participant?.equipment_2 ?? null).filter((token) =>
        ARTIFACT_IDS.has(token.id)
      )
    : [];
  const inscriptionIds = parseSemicolonNumberList(
    participant?.inscriptions ?? null
  );
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
            ...
            {participant?.is_rally ? (
              <Badge>{tCommon("labels.rally")}</Badge>
            ) : null}
          </div>
        </div>
      </div>
        ...
        {showPrimary || showSecondary ? (
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              {showSecondary ? (
                <ReportCommanderRow commander={secondaryCommander} />
              ) : null}
            </div>
          </div>
        ) : null}
